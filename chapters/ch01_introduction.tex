...
\emph{Feasibility.} Before live integration, the forecast asks whether a channel has any defensible operating point under the declared model. The useful comparison is not merely whether a positive tolerance exists, but whether the measured proxy, transfer uncertainty, occupancy, and available suppression can place both stochastic variance and coherent bias inside their gates. The completed archive produces conditional recovery candidates on a subset of the 23 measured channels, negative screening results on others, and a class of cases whose outcome is controlled by measurements the snapshot cadence cannot supply. That mixture --- rather than a blanket claim of recoverability --- is why the detector and the decision framework are both needed.
...
calibration objects, and selection-aware summaries.  The final lower-band
per-epoch anchor export and the health-corrected products remain required, so
the present characterization does not imply that every calibration object is
...
and Fisher chains.  Its conclusions remain conditional until the
pilot-to-allocation, complex-visibility, residual-template, and combined-bin
estimator transfers are measured.
