...
$750{,}461$ detector frames, of which $750{,}457$ satisfy the archived
...
header); and a single detector-kernel cohort, represented by two analyzer
source builds,
...
archived denominator rule does not test.  In 178 rows from eight source events,
the recorded mean baseband power is exactly $128$, the maximum possible value
of $x_r^2+x_i^2$ for signed complex \texttt{int4}.  A frame-wide mean at that
ceiling is consistent with a constant packed \texttt{0x88} fill or a saturated
input path, not ordinary receiver noise, but the raw-source cause has not yet
been adjudicated.  These rows are therefore a named release blocker rather
than silently accepted or silently discarded: the final ledger must attach a
reason-coded input-health decision, and every affected spectrum, distribution,
threshold, and rate must be regenerated after that decision.  The exact
arithmetic checks on the stored rows remain valid; what is pending is whether
the rows are measurements of the sky.  The archive hash, affected event set,
criterion, and required disposition are frozen in
\verb|CANFAR_PRODUCT_HEALTH_AUDIT.json|.
...
\begin{table}[t]
\centering
\caption{Survey inventory fields and their status at the current snapshot.
``Not supplied'' is an evidence state, not a numerical zero.  The final
...
Frames per acquisition & median 3; $750{,}457$ denominator-valid, 4 invalid;
178 ceiling-power rows pending input-health adjudication & Full distribution,
reason-coded validity, and post-health-gate frame counts \\
...
Those values live in the immutable processing ledger and are quoted only where
that ledger supplies them.  Appendix~\ref{app:evidence} records the comparison as measured
