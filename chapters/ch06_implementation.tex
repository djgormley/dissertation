...
\begin{enumerate}
    ...
    \item \emph{Cross-era reproduction.} Earlier development notes state that
    independently built processing cohorts agreed with zero mismatches on an
    overlapping archive subset.  The supplied release cannot reproduce that
    statement: its two source cohorts occupy disjoint channel sets, and no
    joined comparison manifest, event/frame denominator, field list, or build
    pair survives.  The historical statement is therefore not evidence used
    by this dissertation.  What the release does establish is narrower and
    fully auditable: all 23 products record the same supplied kernel digest and
    geometry, each passes its product-contract checks, and source-history
    reconstruction matches the recorded source hash for the 21-product modern
    cohort while explicitly refusing a match for the two-product legacy
    cohort.  A future cross-build equality claim must publish the joined keys,
    completeness, invalid and missing records, exact fields compared, and both
    immutable build manifests.  Under a tolerance regime that comparison would
    require a distributional judgment; under the exactness contract each
    actually joined field reduces to equality.
