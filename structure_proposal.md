...
9. **Masking Cost and Contamination-Residual Tolerance for BAO Cosmology** — proxy and visibility
...
14. **Appendix C: Archive Diagnostics** — the diagnostic construction placed
    beside every physical ATSC channel, with the per-plate atlas counts and the
    measured and refused anchor states for the 23 supplied per-pilot products.

The pre-split source snapshot `legacy/chapters_I-V_draft.tex` is not part of this
bundle; `dissertation.tex` includes only the chapters and appendices listed above.
