...
def fig_claim_chain() -> Path:
    ...
    ax.text(0.02,0.875,r"Each arrow states what must be supplied before the next claim is defensible.",
            transform=ax.transAxes,ha="left",va="top",fontsize=8.0,color=style.MUTED)
    xs=[0.02,0.18,0.34,0.50,0.66,0.82]; ws=[.13,.13,.13,.13,.13,.15]
    boxes=[
        ("Broadcast\nstandard","specified\nfrequency and\nwaveform facts","measured"),
        ("Exact detector","integer projection,\ntransform, and\ndecision","measured"),
        ("Triggered\narchive","recorded detector\nterms and\nprovenance","measured"),
        ("Calibration\ntransfer","pilot to shelf;\npower proxy to\nvisibility residual","model"),
        ("Fisher\nforecast","bias tolerance,\ntime cost, and\nmode cuts","model"),
        ("Channel\nverdict","conditional,\nexcision-shaped,\nor pending","conditional"),
    ]
    for i,((title,body,status),x,w) in enumerate(zip(boxes,xs,ws)):
        style.diagram_box(ax,(x,.49),(w,.27),title=title,body=body,status=status,
                          fontsize=6.75,title_size=7.35)
        if i<len(boxes)-1:
            st="measured" if i<2 else ("model" if i<5 else "conditional")
            style.diagram_arrow(ax,(x+w,.625),(xs[i+1]-.006,.625),status=st)
    ax.annotate(r"load-bearing model gates",xy=(.565,.48),xytext=(.565,.35),
                xycoords=ax.transAxes,textcoords=ax.transAxes,ha="center",fontsize=7.3,
                color=style.MODEL,arrowprops=dict(arrowstyle="-[,widthB=5.2,lengthB=0.45",
                                                  lw=.8,color=style.MODEL))
...
def fig_intro_soundwave() -> Path:
    ...
    titles=[r"initial overdensity",r"pressure launches a shell",r"recombination freezes the ruler"]
    radii=[.10,.52,.76]
    for i,(ax,title,r) in enumerate(zip(axes,titles,radii)):
        ...
        if i==2:
            ax.text(0,-.93,r"$s\simeq150\,\mathrm{Mpc}$",ha="center",fontsize=8.0,color=style.CONDITIONAL)
            ax.plot([0,r],[0,0],color=style.CONDITIONAL,lw=1.2)
        ax.set_title(rf"\textbf{{({chr(97+i)})}} {title}",fontsize=8.5,pad=2)
...
def fig_intro_wiggle() -> Path:
    ...
    r,xi=columns(corr,"separation_mpc","r2_xi"); k,p=columns(power,"k_mpc_inv","p_over_psmooth")
    fig,axes=plt.subplots(1,2,figsize=(style.TEXT_WIDTH,2.85),constrained_layout=True)
    ax=axes[0]; ax.plot(r,xi,color=style.MEASURED)
    peak_r=float(r[np.argmax(xi[(r>120)&(r<175)])+np.where(r>120)[0][0]]) if False else 150.0
    ax.axvline(150,color=style.MODEL,ls=(0,(3,2)),lw=.9)
    ax.annotate(r"BAO bump $\simeq150$ Mpc",xy=(150,np.interp(150,r,xi)),xytext=(112,np.nanmax(xi)*.83),
    ...
    ax.set_title(r"preferred separation",pad=3)
    ax=axes[1]; ax.plot(k,p,color=style.MEASURED)
    ax.axhline(1,color=style.PENDING,lw=.75)
    ax.set_xscale("log"); ax.set_xlim(.035,.5)
    ax.set_xlabel(r"Wavenumber $k$ [Mpc$^{-1}$]"); ax.set_ylabel(r"$P(k)/P_{\rm smooth}(k)$")
    style.clean_axes(ax); style.panel_label(ax,"b"); ax.set_title(r"the same ruler in Fourier space",pad=3)
...
def fig_intro_dilation() -> Path:
    ...
    baseline=f(r); dilated=f(r/1.08); growth=1.18*baseline
    fig,ax=plt.subplots(figsize=(style.TEXT_WIDTH,2.7))
    ax.plot(r,baseline,color=style.INK,label=r"fiducial")
    ax.plot(r,dilated,color=style.MODEL,label=r"dilation: feature shifts")
    ax.plot(r,growth,color=style.CONDITIONAL,ls=(0,(4,2)),label=r"growth: amplitude changes")
    for xx,c in [(150,style.INK),(162,style.MODEL)]: ax.axvline(xx,color=c,lw=.65,ls=(0,(2,2)))
    ax.annotate(r"dilation changes \emph{where} the feature sits",xy=(163,np.interp(163,r,dilated)),
                xytext=(102,np.max(dilated)*.94),fontsize=7.5,color=style.MODEL,
                arrowprops=dict(arrowstyle="->",color=style.MODEL,lw=.7))
    ax.annotate(r"growth changes \emph{how high}",xy=(145,np.interp(145,r,growth)),
                xytext=(88,np.max(growth)*1.09),fontsize=7.5,color=style.CONDITIONAL,
                arrowprops=dict(arrowstyle="->",color=style.CONDITIONAL,lw=.7))
    ax.set_xlim(85,195); ax.set_xlabel(r"Separation [Mpc]"); ax.set_ylabel(r"Correlation amplitude")
    style.clean_axes(ax); ax.legend(loc="upper right",ncol=3,fontsize=7.3,handlelength=2.2)
    ax.set_title(r"Two ways to read the wiggle",pad=4)
...
def fig_intro_band() -> Path:
    ...
    ax.text(600,.67,r"CHIME observing band, 400--800 MHz",ha="center",fontsize=8.1)
    ax.text(539,.46,"North-American DTV\n23 allocations of 6 MHz",ha="center",va="center",fontsize=7.5,color=style.MODEL)
    ax.text(539,.245,r"channels 14--36: archive products + residual chain (complete, Aug 2026)",
            ha="center",va="top",fontsize=6.6,color=style.MODEL)
    ax.annotate(r"the 21-cm line slides through this band",xy=(608,.62),xytext=(674,.83),
                fontsize=7.3,color=style.MEASURED,
...
def fig_intro_two_errors() -> Path:
    ...
    ax.set_ylabel(r"Probability density"); ax.set_yticks([]); style.clean_axes(ax,grid=None)
...
def fig_intro_averaging() -> Path:
    ...
    n=np.logspace(0,6,160)
    thermal=100/np.sqrt(n)
    measured=thermal*np.exp(rng.normal(0,.11,size=n.size))
    measured=np.maximum(measured,.11)
    systematic=np.sqrt(thermal**2+.65**2)
    ...
    ax.axhline(.65,color=style.FAILURE,lw=.75,ls=(0,(2,2)))
    ax.annotate(r"a coherent residual does not average away",xy=(2e5,.67),xytext=(3e3,4.3),
                fontsize=7.4,color=style.MODEL,arrowprops=dict(arrowstyle="->",lw=.7,color=style.MODEL))
    ax.set_xscale("log"); ax.set_yscale("log"); ax.set_xlim(1,1e6)
