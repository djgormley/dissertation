...
def fig_dirichlet_duality() -> Path:
    ...
    ax.annotate(r"adjacent bin: $-3.9$ dB",xy=(1,vals[list(samples).index(1)]),xytext=(1.05,-8),
                fontsize=7.1,color=style.MUTED,arrowprops=dict(arrowstyle="->",color=style.MUTED,lw=.7))
    ax.annotate(r"reference: $-13.5$ dB",xy=(2,vals[list(samples).index(2)]),xytext=(2.05,-25),
                fontsize=7.1,color=style.MODEL,arrowprops=dict(arrowstyle="->",color=style.MODEL,lw=.7))
    ax.set_xlim(-4,4); ax.set_ylim(-42,1); ax.set_xlabel(r"Coarse-grid displacement $\Delta k$ [bins]")
    ...
    ax.text(0,.84,r"designated window",ha="center",fontsize=7.4,color=style.MEASURED)
    ax.text(-2,.10,r"zero",ha="right",fontsize=7.0,color=style.MODEL); ax.text(2,.10,r"zero",ha="left",fontsize=7.0,color=style.MODEL)
    ax.set_xlim(-6,6); ax.set_ylim(-.02,1.03); ax.set_xlabel(r"Padded fine-grid offset [bins]")
