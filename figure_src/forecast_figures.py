...
def fig_bao_footprint() -> Path:
    ...
    ax.annotate(r"ATSC pilot",xy=(pilot,1.15),xytext=(596.6,1.28),fontsize=7.4,color=style.MODEL,
                arrowprops=dict(arrowstyle="->",color=style.MODEL,lw=.7))
    ax.annotate(r"one monitored coarse bin",xy=(centers[pilot_i],.75),xytext=(597.2,.98),fontsize=7.3,color=style.MODEL,
                arrowprops=dict(arrowstyle="->",color=style.MODEL,lw=.7))
    ax.annotate(r"remaining bins inherit the decision" + "\n" + r"only through the shelf-transfer model",xy=(599.4,.44),xytext=(599.15,.83),
                ha="center",fontsize=7.2,color=style.MEASURED,
                arrowprops=dict(arrowstyle="->",color=style.MEASURED,lw=.7))
    ax.set_xlim(lo-.15,hi+.15); ax.set_ylim(-.02,1.38); ax.set_yticks([])
    ax.set_xlabel(r"Frequency [MHz] -- one 6 MHz allocation, 15.36 CHIME coarse channels")
    style.clean_axes(ax,grid=None); style.panel_label(ax,"a"); ax.set_title(r"one allocation: what is measured and what the measurement covers",loc="left",pad=4)
