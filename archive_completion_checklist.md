# Archive and analysis completion checklist — revised 2026-08-26
...
Status update (2026-08-26): PRE-ARCHIVE IMPLEMENTATION IS COMPLETE. The
schema-v5 archive path retains receiver software tag, typed input-map digest,
archive version, collection host, and scope, and refuses nonlocal units missing
the receiver identity. The exact Q16 residual-score bridge and the whole-block
stability assessment are implemented and tested. They still require accepted
v5 archive products and declared empirical controls before they can produce an
authorized channel operating point. The production output has not been
created; launch awaits completion of the current-revision interrupted-resume
gate, CADC certificate renewal, and explicit authorization.

Status update (2026-08-26): A candidate CHORD live-buffer stage now exists on
the unmerged Kotekan `pilot-proxy` feature branch at `097f0bfd`. It binds the
runtime bundle, runs the coarse and calibrated-fine paths, and has synthetic
GPU-oracle and file-pipeline parity coverage. This closes initial framework
coding, not live acceptance: calibration, branch adoption, explicit validity
and input-health products, downstream application, full-load timing, shadow
operation, monitoring, and failure injection remain open.
...
holdout (item G). The coarse threshold-free estimator transfer is complete, but
the current-geometry fine-statistic sensitivity, threshold, and numerical
representation-loss study (item H) remains mandatory before the dissertation
claims a measured current-geometry detection sensitivity. The sign-off
...
- ch 32 (568): one 12-h session at 1 frame/30 s plus three 30-s continuous
  stretches; refines the modern-era ≤5-min bound with gap-free lags.
...
Fixed forward (2026-08-21): per-pilot product schema v2 introduced the exact
terms needed to close the Q16 retention gap. The current approved reprocessing
emits schema v5 products carrying `fine_power_u64`, the three exact uint64
fine-power terms per bin taken from the frozen fixed-point transform, alongside
the unsummed lower/upper reference powers. The v1 products retained only a
float32 ratio formed with a floating FFT, which is neither the deployed
statistic nor invertible to it, so the frozen fine decision could not be
evaluated on them at any threshold. This does not retroactively unblock the
item: the existing archive must be reprocessed for the terms to exist, and the
blinding requirement above is unchanged. The approved v5 reprocessing can
support the exact per-era Q16 bundle once its products exist.

Fixed forward (2026-08-26): each nonlocal v5 unit also retains the receiver
archive version, receiver software tag, and typed input-map digest. Collection
host and archive scope are recorded separately. The residual-score builder now
validates this provenance and converts the retained fine powers into exact
all-rank Q16 score arrays with source and producer hashes. The stability path
resamples whole acquisition or sidereal-day blocks, stratified by era half, and
can refuse an incomplete or unbounded candidate surface. These code paths close
the preparation gap but not the evidence gap: the production archive has not
run, and the resampling controls and acceptable margins remain unset pending
the declared empirical basis.

For every channel and accepted station-state era, release together:
...
**Status: partially complete in code; no new telescope observation is
required.** The released fully digital and controlled radio-path sweeps close
threshold-free transfer for the coarse estimator. They do not close this item,
which still requires the current full-$2048$ fine statistic, threshold sweep,
offset study, and stage-by-stage numerical representation loss. The current
23-profile weights, standards-chain generator, packed sample path, fixed
transform, supplied v2.1 kernel, CPU reference, and an RTX-class GPU are
available. The final detection release is accepted only if its predeclared
crossing, multi-seed, false-alarm-tail, uncertainty, clipping, and literal
full-frame audit gates pass; a partially completed sweep is not promoted.
...
- Make the primary science product a rank-indexed mask-versus-proxy-residual
  frontier. Hold the detector rank fixed on each curve, label the measured
  points by threshold multiplier, and retain the empirical steps without
  smoothing. Publish the inverted tolerance-to-minimum-mask curves and their
  envelope across ranks. Apply the full residual-plus-masking cost when choosing
  an operating point; treat the conventional ROC as a supporting diagnostic.
...
**Status: an unmerged CHORD feature implementation exists; target-telescope
acceptance still requires the actual `kotekan` environment, telescope-load
test, and operations authority.**  Repository tests establish synthetic
arithmetic and transport parity, but archive products and file harnesses cannot
measure live contention, recovery, latency, drift response, or incident capture.

- Merge or otherwise adopt the feature stage and re-vendor its PilotProxy core
  from a resolvable upstream release or restored source snapshot.
- Complete the Chapter 10 reason-coded validity, independent input-health,
  monitoring, and downstream mask-application surfaces on the target buffer path.
...
- Run for multiple hours in shadow mode with one frozen calibration bundle.
  Record proposed decisions without applying them to science data, replay a
  sampled subset through the independent reference, and compare locked
  time-block summaries with the offline holdout prediction. An applied-mask
  trial follows only after the shadow gates pass.
...
- Restore archival refs or exact source snapshots for the PilotProxy commits
  named by immutable figures and evidence releases; verify reachability rather
  than changing their historical producer identities.
