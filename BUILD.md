...
On this machine that environment is `~/.venvs/dissertation` (`~/.venvs/pilot-proxy`
also carries Basemap and produces byte-identical figures). The Python `make`
defaults to does not, and `figure_src.broadcast_figures` imports Basemap at module
load, so without the override the figure step aborts before producing anything:

    make verify PYTHON=~/.venvs/dissertation/bin/python

The gate-only targets (`test`, `evidence-audit`, `manifest-check`) have no
Basemap dependency and run under any Python 3.
